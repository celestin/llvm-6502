...
static bool CC_Mos6502_Assign_SRet(unsigned &ValNo, MVT &ValVT,
...
static bool CC_Mos6502_Assign_f64(unsigned &ValNo, MVT &ValVT,
                                MVT &LocVT, CCValAssign::LocInfo &LocInfo,
                                ISD::ArgFlagsTy &ArgFlags, CCState &State)
{
...
static bool CC_Mos650264_Full(unsigned &ValNo, MVT &ValVT,
                            MVT &LocVT, CCValAssign::LocInfo &LocInfo,
                            ISD::ArgFlagsTy &ArgFlags, CCState &State) {
...
static bool CC_Mos650264_Half(unsigned &ValNo, MVT &ValVT,
                            MVT &LocVT, CCValAssign::LocInfo &LocInfo,
                            ISD::ArgFlagsTy &ArgFlags, CCState &State) {
...
// The calling conventions in Mos6502CallingConv.td are described in terms of the
...
SDValue
Mos6502TargetLowering::LowerReturn(SDValue Chain,
...
SDValue
Mos6502TargetLowering::LowerReturn_32(SDValue Chain,
                                    CallingConv::ID CallConv, bool IsVarArg,
                                    const SmallVectorImpl<ISD::OutputArg> &Outs,
                                    const SmallVectorImpl<SDValue> &OutVals,
                                    SDLoc DL, SelectionDAG &DAG) const {
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_Mos650232);
...
SDValue
Mos6502TargetLowering::LowerReturn_64(SDValue Chain,
                                    CallingConv::ID CallConv, bool IsVarArg,
                                    const SmallVectorImpl<ISD::OutputArg> &Outs,
                                    const SmallVectorImpl<SDValue> &OutVals,
                                    SDLoc DL, SelectionDAG &DAG) const {
  ...
  // Analyze return values.
  CCInfo.AnalyzeReturn(Outs, RetCC_Mos650264);
...
LowerFormalArguments_32(SDValue Chain,
  ...
  CCState CCInfo(CallConv, isVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_Mos650232);
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i, ++InIdx) {
    ...
    if (Ins[InIdx].Flags.isSRet()) {
      if (InIdx != 0)
        report_fatal_error("mos6502 only supports sret on the first parameter");
      // Get SRet from [%fp+64].
    ...
    if (VA.getValVT() == MVT::i32 || VA.getValVT() == MVT::f32) {
      ...
      // Mos6502 is big endian, so add an offset based on the ObjectVT.
...
LowerFormalArguments_64(SDValue Chain,
  ...
  // Analyze arguments according to CC_Mos650264.
  SmallVector<CCValAssign, 16> ArgLocs;
  CCState CCInfo(CallConv, IsVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_Mos650264);
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    assert(VA.isMemLoc());
    // The CC_Mos650264_Full/Half functions compute stack offsets relative to the
    // beginning of the arguments area at %fp+BIAS+128.
    unsigned Offset = VA.getLocMemOffset() + ArgArea;
    unsigned ValSize = VA.getValVT().getSizeInBits() / 8;
    // Adjust offset for extended arguments, MOS6502 is big-endian.
...
SDValue
Mos6502TargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
                               SmallVectorImpl<SDValue> &InVals) const {
  if (Subtarget->is64Bit())
...
SDValue
Mos6502TargetLowering::LowerCall_32(TargetLowering::CallLoweringInfo &CLI,
                                  SmallVectorImpl<SDValue> &InVals) const {
  ...
  // Mos6502 target does not yet support tail call optimization.
  isTailCall = false;
  ...
  CCState CCInfo(CallConv, isVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeCallOperands(Outs, CC_Mos650232);
  ...
  for (unsigned i = 0, realArgIdx = 0, byvalArgIdx = 0, e = ArgLocs.size();
    ...
    if (VA.needsCustom()) {
      ...
      SDValue Store = DAG.getStore(DAG.getEntryNode(), dl,
                                   ...
                                   false, false, 0);
      // Mos6502 is big-endian, so the high part comes first.
  ...
  unsigned TF = ((getTargetMachine().getRelocationModel() == Reloc::PIC_)
                 ? Mos6502MCExpr::VK_Mos6502_WPLT30 : 0);
  if (GlobalAddressSDNode *G = dyn_cast<GlobalAddressSDNode>(Callee))
  ...
  const Mos6502RegisterInfo *TRI = Subtarget->getRegisterInfo();
  const uint32_t *Mask =
      ((hasReturnsTwice)
  ...
  RVInfo.AnalyzeCallResult(Ins, RetCC_Mos650232);
...
unsigned
Mos6502TargetLowering::getSRetArgSize(SelectionDAG &DAG, SDValue Callee) const
{
  const Function *CalleeFn = nullptr;
  if (GlobalAddressSDNode *G = dyn_cast<GlobalAddressSDNode>(Callee)) {
    CalleeFn = dyn_cast<Function>(G->getGlobal());
...
//
// The MOS6502 v9 ABI requires that floating point arguments are treated the same
...
SDValue
Mos6502TargetLowering::LowerCall_64(TargetLowering::CallLoweringInfo &CLI,
                                  SmallVectorImpl<SDValue> &InVals) const {
  SelectionDAG &DAG = CLI.DAG;
  SDLoc DL = CLI.DL;
  SDValue Chain = CLI.Chain;
  ...
  CCState CCInfo(CLI.CallConv, CLI.IsVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeCallOperands(CLI.Outs, CC_Mos650264);

  // Get the size of the outgoing arguments stack space requirement.
  // The stack offset computed by CC_Mos650264 includes all arguments.
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    case CCValAssign::BCvt:
      ...
      // MOS6502 does not support i128 natively. Lower it into two i64, see below.
  ...
  RVInfo.AnalyzeCallResult(CLI.Ins, RetCC_Mos650264);
...
/// IntCondCCodeToICC - Convert a DAG integer condition code to a MOS6502 ICC
/// condition.
...
/// FPCondCCodeToFCC - Convert a DAG floatingp oint condition code to a MOS6502
/// FCC condition.
...
Mos6502TargetLowering::Mos6502TargetLowering(TargetMachine &TM,
  ...
  // Mos6502 doesn't have i1 sign extending load
  ...
  // Mos6502 doesn't have sext_inreg, replace them with shl/sra
  setOperationAction(ISD::SIGN_EXTEND_INREG, MVT::i16, Expand);
  setOperationAction(ISD::SIGN_EXTEND_INREG, MVT::i8 , Expand);
  setOperationAction(ISD::SIGN_EXTEND_INREG, MVT::i1 , Expand);

  // Mos6502 has no REM or DIVREM operations.
  ...
  // Mos6502 has no select or setcc: expand to SELECT_CC.
  ...
  // Mos6502 doesn't have BRCOND either, it has BR_CC.
  ...
  if (!Subtarget->isV9()) {
    // Mos6502V8 does not have FNEGD and FABSD.
  ...
  // FIXME: Mos6502 provides these multiplies, but we don't have them yet.
...
EVT Mos6502TargetLowering::getSetCCResultType(const DataLayout &, LLVMContext &,
                                            EVT VT) const {
  if (!VT.isVector())
    return MVT::i32;
...
/// combiner.
void Mos6502TargetLowering::computeKnownBitsForTargetNode
                                (const SDValue Op,
                                 APInt &KnownZero,
                                 APInt &KnownOne,
...
SDValue Mos6502TargetLowering::withTargetFlags(SDValue Op, unsigned TF,
...
SDValue Mos6502TargetLowering::makeHiLoPair(SDValue Op,
...
SDValue Mos6502TargetLowering::makeAddress(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  if (getTargetMachine().getRelocationModel() == Reloc::PIC_) {
    ...
    SDValue HiLo = makeHiLoPair(Op, Mos6502MCExpr::VK_Mos6502_GOT22,
                                Mos6502MCExpr::VK_Mos6502_GOT10, DAG);
  ...
  case CodeModel::Small:
    // abs32.
    return makeHiLoPair(Op, Mos6502MCExpr::VK_Mos6502_HI,
                        Mos6502MCExpr::VK_Mos6502_LO, DAG);
  case CodeModel::Medium: {
    // abs44.
    SDValue H44 = makeHiLoPair(Op, Mos6502MCExpr::VK_Mos6502_H44,
                               Mos6502MCExpr::VK_Mos6502_M44, DAG);
    H44 = DAG.getNode(ISD::SHL, DL, VT, H44, DAG.getConstant(12, DL, MVT::i32));
    SDValue L44 = withTargetFlags(Op, Mos6502MCExpr::VK_Mos6502_L44, DAG);
    L44 = DAG.getNode(SPISD::Lo, DL, VT, L44);
    return DAG.getNode(ISD::ADD, DL, VT, H44, L44);
  }
  case CodeModel::Large: {
    // abs64.
    SDValue Hi = makeHiLoPair(Op, Mos6502MCExpr::VK_Mos6502_HH,
                              Mos6502MCExpr::VK_Mos6502_HM, DAG);
    Hi = DAG.getNode(ISD::SHL, DL, VT, Hi, DAG.getConstant(32, DL, MVT::i32));
    SDValue Lo = makeHiLoPair(Op, Mos6502MCExpr::VK_Mos6502_HI,
                              Mos6502MCExpr::VK_Mos6502_LO, DAG);
    return DAG.getNode(ISD::ADD, DL, VT, Hi, Lo);
  }
  }
...
SDValue Mos6502TargetLowering::LowerConstantPool(SDValue Op,
                                               SelectionDAG &DAG) const {
  return makeAddress(Op, DAG);
}

SDValue Mos6502TargetLowering::LowerBlockAddress(SDValue Op,
                                               SelectionDAG &DAG) const {
  return makeAddress(Op, DAG);
}

SDValue Mos6502TargetLowering::LowerGlobalTLSAddress(SDValue Op,
                                                   SelectionDAG &DAG) const {
  ...
  if (model == TLSModel::GeneralDynamic || model == TLSModel::LocalDynamic) {
    unsigned HiTF = ((model == TLSModel::GeneralDynamic)
                     ? Mos6502MCExpr::VK_Mos6502_TLS_GD_HI22
                     : Mos6502MCExpr::VK_Mos6502_TLS_LDM_HI22);
    unsigned LoTF = ((model == TLSModel::GeneralDynamic)
                     ? Mos6502MCExpr::VK_Mos6502_TLS_GD_LO10
                     : Mos6502MCExpr::VK_Mos6502_TLS_LDM_LO10);
    unsigned addTF = ((model == TLSModel::GeneralDynamic)
                      ? Mos6502MCExpr::VK_Mos6502_TLS_GD_ADD
                      : Mos6502MCExpr::VK_Mos6502_TLS_LDM_ADD);
    unsigned callTF = ((model == TLSModel::GeneralDynamic)
                       ? Mos6502MCExpr::VK_Mos6502_TLS_GD_CALL
                       : Mos6502MCExpr::VK_Mos6502_TLS_LDM_CALL);
    ...
    SDValue Hi = DAG.getNode(SPISD::Hi, DL, PtrVT,
                 withTargetFlags(Op, Mos6502MCExpr::VK_Mos6502_TLS_LDO_HIX22, DAG));
    SDValue Lo = DAG.getNode(SPISD::Lo, DL, PtrVT,
                 withTargetFlags(Op, Mos6502MCExpr::VK_Mos6502_TLS_LDO_LOX10, DAG));
    HiLo =  DAG.getNode(ISD::XOR, DL, PtrVT, Hi, Lo);
    return DAG.getNode(SPISD::TLS_ADD, DL, PtrVT, Ret, HiLo,
                   withTargetFlags(Op, Mos6502MCExpr::VK_Mos6502_TLS_LDO_ADD, DAG));
  }

  if (model == TLSModel::InitialExec) {
    unsigned ldTF     = ((PtrVT == MVT::i64)? Mos6502MCExpr::VK_Mos6502_TLS_IE_LDX
                         : Mos6502MCExpr::VK_Mos6502_TLS_IE_LD);
    ...
    SDValue TGA = makeHiLoPair(Op,
                               Mos6502MCExpr::VK_Mos6502_TLS_IE_HI22,
                               Mos6502MCExpr::VK_Mos6502_TLS_IE_LO10, DAG);
  ...
  SDValue Hi = DAG.getNode(SPISD::Hi, DL, PtrVT,
                  withTargetFlags(Op, Mos6502MCExpr::VK_Mos6502_TLS_LE_HIX22, DAG));
  SDValue Lo = DAG.getNode(SPISD::Lo, DL, PtrVT,
                  withTargetFlags(Op, Mos6502MCExpr::VK_Mos6502_TLS_LE_LOX10, DAG));
...
SDValue
Mos6502TargetLowering::LowerF128_LibCallArg(SDValue Chain, ArgListTy &Args,
...
SDValue
Mos6502TargetLowering::LowerF128Op(SDValue Op, SelectionDAG &DAG,
...
SDValue
Mos6502TargetLowering::LowerF128Compare(SDValue LHS, SDValue RHS,
                                      unsigned &SPCC,
                                      SDLoc DL,
                                      SelectionDAG &DAG) const {
...
// Custom lower UMULO/SMULO for MOS6502. This code is similar to ExpandNode()
...
MachineBasicBlock*
Mos6502TargetLowering::expandSelectCC(MachineInstr *MI,
...
MachineBasicBlock*
Mos6502TargetLowering::expandAtomicRMW(MachineInstr *MI,
...
//===----------------------------------------------------------------------===//
//                         Mos6502 Inline Assembly Support
//===----------------------------------------------------------------------===//

/// getConstraintType - Given a constraint letter, return the type of
/// constraint it is for this target.
Mos6502TargetLowering::ConstraintType
...
TargetLowering::ConstraintWeight Mos6502TargetLowering::
...
Mos6502TargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
...
bool
Mos6502TargetLowering::isOffsetFoldingLegal(const GlobalAddressSDNode *GA) const {
  // The Mos6502 target isn't yet aware of offsets.
  return false;
}
